...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-primary">Modifier la Séance</h1>
            ...
            <form action="{{ route('admin.seances.update', $seance->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="ue_id" name="ue_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('ue_id') border-red-500 @enderror" required>
                            ...
                            @foreach ($ues as $ue)
                                <option value="{{ $ue->id }}" {{ old('ue_id', $seance->ue_id) == $ue->id ? 'selected' : '' }}>
                                    {{ $ue->nom }} ({{ $ue->code }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" id="jour" name="jour" value="{{ old('jour', $seance->jour) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('jour') border-red-500 @enderror" required>
                        @error('jour')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="time" id="heure_debut" name="heure_debut" value="{{ old('heure_debut', \Carbon\Carbon::parse($seance->heure_debut)->format('H:i')) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('heure_debut') border-red-500 @enderror" required>
                        @error('heure_debut')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="time" id="heure_fin" name="heure_fin" value="{{ old('heure_fin', \Carbon\Carbon::parse($seance->heure_fin)->format('H:i')) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('heure_fin') border-red-500 @enderror" required>
                        @error('heure_fin')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select id="salle_id" name="salle_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('salle_id') border-red-500 @enderror" required>
                            ...
                            @foreach ($salles as $salle)
                                <option value="{{ $salle->id }}" data-capacite="{{ $salle->capacite }}" {{ old('salle_id', $seance->salle_id) == $salle->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select id="groupe_id" name="groupe_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('groupe_id') border-red-500 @enderror" required>
                            ...
                            @foreach ($groupes as $groupe)
                                <option value="{{ $groupe->id }}" data-effectif="{{ $groupe->effectifs->first()?->effectif ?? 0 }}" {{ old('groupe_id', $seance->groupe_id) == $groupe->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select id="enseignant_id" name="enseignant_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('enseignant_id') border-red-500 @enderror" required>
                            ...
                            @foreach ($teachers as $teacher)
                                <option value="{{ $teacher->id }}" {{ old('enseignant_id', $seance->enseignant_id) == $teacher->id ? 'selected' : '' }}>
                ...
                <div class="flex items-center justify-end mt-6">
                    <button type="submit" class="bg-primary text-white px-4 py-2 rounded-lg font-medium hover:bg-accent transition-colors">
                        <i class="fas fa-save mr-2"></i>Mettre à jour la séance
                    </button>
                </div>
            </form>
        </div>
